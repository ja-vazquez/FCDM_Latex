...
% 
%
Because there is no reason to prefer a Universe with a critical density, hence
...
a definite value. This means that  these quantum fluctuation become classical once they have 
...
\end{equation}
independent of $(m_{\rm Pl}/\mu)$. In the other side the tensor-to-scalar ratio for this model is given by \textcolor{red}{(ecuaci\'on 28.10 del Liddle)}
\begin{equation}
r = 8\left(\frac{\mu}{M_{pl}}\right)^{2p/(p-2)}\left(\frac{p}{2N(p-2)}\right)^{2(p-1)/(p-2)}
\end{equation}

%In addition, when $\mu < m_{\rm Pl}$ 
...
- \epsilon$ \textcolor{red}{Duda: C\'omo puede ser esto si $\eta$ es lineal en $V''$ y se est\'a pidiendo que $V''=0$?}. The spectral index and tensor-to-scalar ratio are given by 
\beq
...
%
For this potential, the inflationary parameters are \textcolor{red}{Me bas\'e en la p\'agina 459 de Liddle}
%
\bea
