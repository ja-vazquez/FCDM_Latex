...
%For simplicity we follow
 \textcolor{red}{[Christian T. Byrnes, David Wands;  Phys.Rev. D74 (2006) 043529; 
 ...
 The generalization to more than two fields can be easily obtained and described by
  \textcolor{red}{[https://arxiv.org/pdf/1606.06971.pdf]}. 
...
a new set of slow-roll parameters, defined as \jav{cambie la G por masas de planck, pero revisar que el texto
de abajo sea consistente}

\begin{equation}\label{slowroll2f}
\epsilon_i = \frac{m_{Pl}^2}{16 \pi }\left(\frac{V_i}{V}\right)^2, \ \ \ \ \ \ \ \ 
\eta_{ij}=\frac{m_{Pl}^2}{8\pi G}\left(\frac{V_{ij}}{V}\right).
\end{equation}
...
\noindent
On the largest scales ($k\ll aH$) it is better to work on a rotating basis of the fields given by
  \begin{subequations}
  \begin{equation}
...
The field $\sigma$ is parallel to the trajectory of the field space and usually called 
\textit{adiabatic field}, whereas the field  $s$ is perpendicular and named it the \textit{entropy field}. 
If the background trajectory is curved then $\delta\sigma$ and $\delta s$ are correlated at Hubble exit,
and therefore at this point  the power spectrum and cross-correlation are given by
...
\end{equation}
\end{subequations}
where $C\simeq 0.7296$ and $\epsilon\equiv \epsilon_{\sigma\sigma}+\epsilon_{\eta\eta}$ and $\eta_{ij}$ ($i,j=\sigma,s$) are 
...
The curvature and isocurvature perturbations are now defined as
...
perturbations can be written using the formalism of tranfer matrix as
\begin{equation}
...
\end{equation}
with $\Delta$ is the observable correlation angle defined at lower order as
\begin{equation}
...
In order to understand the contribution of each field to the primordial spectrum, 
the primordial adiabatic and entropy perturbations on super-horizon scales are commonly parameterised as power laws
...
Given the fact that scalar and tensor perturbations are decoupled at linear order, 
 gravitational waves at horizon crossing 
 ...
 the tensor power spectrum and the spectral index are given by
\begin{equation}
...
the curvature perturbations continue evolving as \eqref{PRf}. In this way the tensor-to-scalar ratio some time after the end of inflation is
\begin{equation}\label{Tensortoscalar}
...
\textcolor{red}{[http://iopscience.iop.org/article/10.1088/0264-9381/28/7/075009/pdf]}
\begin{equation}
